  ...
  try {
    ...
    res
      ...
      .json({
        ...
        newCreditBalance: client.credit,
      });
  } catch (error) {
    console.error("Error subtracting credits:", error);
    res
      .status(500)
      .json({ error: "An error occurred while subtracting credits" });
  }
};
...
exports.getbygroupid = async (req, res) => {
  try {
    ...
    console.log("groupid:", groupid); // Log the value for debugging
    const client = await Client.findOne({ groupid: groupid });
